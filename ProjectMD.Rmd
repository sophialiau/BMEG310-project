...
# Plot dendrogram
plot(hc, labels = FALSE, main = "Hierarchical Clustering of LIHC (Top 500 HVGs)")
...
# 1. Select top 500 HVGs
var_genes <- rowVars(vst_mat)
hvg_index <- order(var_genes, decreasing = TRUE)[1:500]
hvg_mat <- vst_mat[hvg_index, ]

# 2. PCA on HVGs
pca_hvg <- prcomp(t(hvg_mat), scale. = TRUE)

# Inspect variance explained
summary(pca_hvg)

pcs_for_clust <- pca_hvg$x[, 1:30]

# Distance on PCA space (samples × PCs)
dist_pca <- dist(pcs_for_clust)

# Hierarchical clustering
hc_pca <- hclust(dist_pca, method = "ward.D2")

# Plot dendrogram
plot(hc_pca, labels = FALSE, main = "Hierarchical Clustering on PCA Space")

# Choose number of clusters
k <- 3
clusters <- cutree(hc_pca, k = k)
...
clinical_expr$Stage_Grouped <- ifelse(grepl("^STAGE I$",   clinical_expr$Stage), "I",
                               ...
                               ifelse(grepl("^STAGE III", clinical_expr$Stage), "III/IV",
                               ifelse(grepl("^STAGE IV",  clinical_expr$Stage), "III/IV",
                                      NA))))
...
```{r}
# Remove NA
surv_df <- clinical_expr %>%
  filter(!is.na(Overall_Time), !is.na(Patient_ID))

median_os <- median(surv_df$Overall_Time, na.rm = TRUE)

surv_df$Surv_Group <- ifelse(
  surv_df$Overall_Time > median_os, "High", "Low"
)

table(surv_df$Surv_Group)
```

```{r}
library(EnhancedVolcano)
# Subset count matrix to only patients with survival info
common_ids_surv <- intersect(colnames(count_mat_filt), surv_df$Patient_ID)

count_surv <- count_mat_filt[, common_ids_surv]
surv_df2 <- surv_df[match(common_ids_surv, surv_df$Patient_ID), ]

stopifnot(all(colnames(count_surv) == surv_df2$Patient_ID))

dds_surv <- DESeqDataSetFromMatrix(
  countData = count_surv,
  colData   = data.frame(Survival = factor(surv_df2$Surv_Group)),
  design    = ~ Survival
)

dds_surv <- DESeq(dds_surv)
res_surv <- results(dds_surv, contrast = c("Survival", "High", "Low"))
res_surv <- res_surv[order(res_surv$padj), ]

EnhancedVolcano(res_surv,
                lab = rownames(res_surv),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'High vs Low Overall Survival')
```
...
#count matrix or csv
rnaseq$Gene_clean <- sub("\\..*$", "", rnaseq$Gene)
...
```{r}
expr_mut_merge <- merge(
  clinical_expr[, c("Patient_ID", "Expression_Cluster")],
  clinical2[, c("Patient_ID", "Mutation_Cluster")],
  by = "Patient_ID"
)

# Convert to factor
expr_mut_merge$Expression_Cluster <- factor(expr_mut_merge$Expression_Cluster)
expr_mut_merge$Mutation_Cluster   <- factor(expr_mut_merge$Mutation_Cluster)

# Check counts
table(expr_mut_merge$Expression_Cluster)
table(expr_mut_merge$Mutation_Cluster)

## -------------------------------
## 1. Contingency Table
## -------------------------------
tab_expr_mut <- table(expr_mut_merge$Expression_Cluster,
                      expr_mut_merge$Mutation_Cluster)
...
if(any(tab_expr_mut < 5)) {
  test_expr_mut <- fisher.test(tab_expr_mut)
} else {
  test_expr_mut <- chisq.test(tab_expr_mut)
}
...
```{r}
prop_expr_mut <- prop.table(tab_expr_mut, margin = 1)
...
barplot(t(prop_expr_mut),
        ...
        ylab        = "Proportion",
        main        = "Mutation-Cluster Composition Within Each Expression Cluster")
```

```{r}
library(ggplot2)

ggplot(expr_mut_merge, aes(x = Expression_Cluster,
                           fill = Mutation_Cluster)) +
  geom_bar(position = "fill") +
  labs(title = "Mutation Cluster Distribution Across Expression Clusters",
...
```{r}
common_ids_sex <- intersect(colnames(count_mat_filt), clinical_expr$Patient_ID)

count_sex <- count_mat_filt[, common_ids_sex]
clin_sex <- clinical_expr[match(common_ids_sex, clinical_expr$Patient_ID), ]

stopifnot(all(colnames(count_sex) == clin_sex$Patient_ID))

# Ensure Sex is a factor
clin_sex$Sex <- factor(clin_sex$Sex)
table(clin_sex$Sex)
```
...
dds_sex <- DESeqDataSetFromMatrix(
  countData = count_sex,
  colData   = data.frame(Sex = clin_sex$Sex),
  design    = ~ Sex
)
...
```{r}
clin_age <- clinical_expr %>%
  filter(!is.na(Diagnosis.Age))

median_age <- median(clin_age$Diagnosis.Age, na.rm = TRUE)

clin_age$Age_Group <- ifelse(
  clin_age$Diagnosis.Age > median_age, "Old", "Young"
)

table(clin_age$Age_Group)
clin_age$Age_Group <- factor(clin_age$Age_Group)
```

```{r}
common_ids_age <- intersect(colnames(count_mat_filt), clin_age$Patient_ID)

count_age <- count_mat_filt[, common_ids_age]
clin_age2 <- clin_age[match(common_ids_age, clin_age$Patient_ID), ]

stopifnot(all(colnames(count_age) == clin_age2$Patient_ID))
```

```{r}
dds_age <- DESeqDataSetFromMatrix(
  countData = count_age,
  colData   = data.frame(Age_Group = clin_age2$Age_Group),
  design    = ~ Age_Group
)

dds_age <- DESeq(dds_age)

res_age <- results(dds_age, contrast = c("Age_Group", "Old", "Young"))
res_age <- res_age[order(res_age$padj), ]
head(res_age)
EnhancedVolcano(
  res_age,
  lab = rownames(res_age),
  x = 'log2FoldChange',
  y = 'padj',
