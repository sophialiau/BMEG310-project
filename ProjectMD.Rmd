...
```{r}
dist_mat <- dist(t(mut_matrix), method = "euclidean")
hc <- hclust(dist_mat, method = "ward.D2")
plot(hc, labels = FALSE, main = "Patient Clustering Based on Mutation Matrix")
clusters <- cutree(hc, k = 2)
clinical$Mutation_Cluster <- clusters[clinical$Patient_ID]
```

```{r}
os_data <- clinical[, c("Overall_Time", "Overall_Status", "Mutation_Cluster")]
...
```{r}
dss_data <- clinical[, c("DSS_Time", "DSS_Status", "Mutation_Cluster")]
...
```{r}
dfs_data <- clinical[, c("DFS_Time", "DFS_Status", "Mutation_Cluster")]
...
```{r}
pfs_data <- clinical[, c("PFS_Time", "PFS_Status", "Mutation_Cluster")]
