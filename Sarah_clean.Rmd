...
genes_for_model <- gene_diff_tbl$Gene[1:20]

genes_for_model <- intersect(genes_for_model, colnames(train))
...
gene_freq <- rowSums(mut_matrix)
genes_for_model_2 <- names(sort(gene_freq, decreasing = TRUE))[1:20]
...
run_nb_once <- function(seed, df, genes_for_model_2, threshold = 0.5) {
  set.seed(seed)
...
nb_results <- purrr::map_dfr(
  seeds,
  ~run_nb_once(.x, df = df, genes_for_model_2 = genes_for_model_2, threshold = 0.5)
)
