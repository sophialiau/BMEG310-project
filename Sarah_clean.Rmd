...
genes_for_model_2 <- intersect(genes_for_model_2, colnames(train))
train_nb <- train[, c(genes_for_model_2, "Stage_lowhigh")]


run_nb_once <- function(seed, df, genes_for_model_2, threshold = 0.999) {
  set.seed(seed)
  ...
  nb_prob <- predict(
    nb_model,
    newdata = test[, genes_for_model_2, drop = FALSE],
    type = "raw"
  )
...
nb_results <- purrr::map_dfr(
  seeds,
  ~run_nb_once(.x, df = df, genes_for_model_2 = genes_for_model_2, threshold = 0.999)
)
