...
fgsea_1v2 <- fgsea_1v2[order(fgsea_1v2$padj), ]
head(fgsea_1v2, 10)
...
```{r}
# Filter significant pathways and order by NES
fg_sig <- fgsea_1v2 %>%
  dplyr::filter(padj < 0.05) %>%
  dplyr::mutate(direction = ifelse(NES > 0, "Up in Cluster 2", "Down in Cluster 2")) %>%
  dplyr::arrange(NES)
...
ggplot(fg_sig,
  ...
  coord_flip() +
  labs(
    title = "Cluster 2 vs 1 – Hallmark Pathway Enrichment",
    x     = "Hallmark Pathway",
    y     = "NES"
  ) +
  scale_fill_manual(values = c("Up in Cluster 2" = "#1f78b4",
                               "Down in Cluster 2" = "#e31a1c")) +
  theme_minimal(base_size = 10)

summary(fgsea_1v2$NES)
table(sign(fgsea_1v2$NES[fgsea_1v2$padj < 0.05]))
...
sig_paths <- fgsea_1v2 %>%
  as_tibble() %>%                               # make sure it's a plain tibble
  dplyr::filter(padj < 0.05, NES > 0) %>%       # use dplyr filter
  dplyr::arrange(padj) %>%                      # order by padj
  dplyr::slice(1:3)                             # top 3

# build edge list only for leading-edge genes
edge_df <- sig_paths %>%
...
# identify node types
V(g)$type <- ifelse(V(g)$name %in% sig_paths$pathway, "pathway", "gene")
...
ggraph(g, layout = "fr") +
  ...
  scale_color_manual(values = c("pathway" = "#E76F51", "gene" = "#2A9D8F")) +
  scale_size_manual(values = c("pathway" = 8, "gene" = 3)) +
  theme_void() +
  labs(title = "Pathway–Gene Network: Upregulated in Cluster 2")
