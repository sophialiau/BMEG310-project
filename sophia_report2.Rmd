...
summary(fgsea_2v1$NES)
table(sign(fgsea_2v1$NES[fgsea_2v1$padj < 0.05]))
...
# pick 3 significant pathways (NES > 0, strong upregulated signals)
sig_paths <- fgsea_2v1 %>%
  filter(padj < 0.05, NES > 0) %>%
  arrange(padj) %>%
  slice(1:3)   # CHANGE if you want more pathways
